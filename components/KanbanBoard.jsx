...
const extractNumber = (text) => {
  const wordToNum = {
    ...
    'to': 2, 'too': 2, 'for': 4, // Common misheard words
  };
  
  const lower = text.toLowerCase();
  
  // Check for "number X" pattern first
  const numberMatch = lower.match(/number\s+(\w+)/);
  if (numberMatch) {
    const numWord = numberMatch[1];
    if (wordToNum[numWord]) return wordToNum[numWord];
    const parsed = parseInt(numWord, 10);
    if (!isNaN(parsed)) return parsed;
  }
  
  // Check for "task X" pattern
  const taskMatch = lower.match(/task\s+(\w+)/);
  if (taskMatch) {
    const numWord = taskMatch[1];
    if (wordToNum[numWord]) return wordToNum[numWord];
    const parsed = parseInt(numWord, 10);
    if (!isNaN(parsed)) return parsed;
  }
...
// ARGUS Listening System - with debounced command execution
const useArgusListener = (onCommand) => {
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event) => {
      ...
      // Check if contains wake word "argus"
      if (lower.includes('argus') || lower.includes('ar gus')) {
        // Extract command after "argus"
        ...
        if (command && command !== lastProcessedRef.current) {
          ...
          // Set new timeout - execute after 1.5 seconds of no new speech
    ...
    recognition.onend = () => {
      // Process any pending command immediately when speech ends
      ...
      {showVoiceSettings && (
        <div style={styles.voiceSettingsPanel}>
          ...
          <div style={styles.commandHints}>
            <strong>Say:</strong> "Argus, move task 2 to done" • "Argus, status" • "Argus, what is task 3" • "Argus, block task 7" • "Argus, focus"
          </div>
        </div>
      )}
