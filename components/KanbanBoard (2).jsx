...
const ARGUS_INSIGHTS = {
  ...
  taskInfo: [
    "Task {number} is {task}. Status: {status}. Impact: {impact}.",
  ],
};
...
const useArgusListener = (onCommand) => {
  ...
  return { isListening, isAwake, transcript, startListening, stopListening };
};
...
export default function KanbanBoard() {
  ...
  // Create numbered task map - numbers are assigned by column order then creation date
  const numberedTasks = useMemo(() => {
    const ordered = [];
    // Order: Ready, In Progress, Review, Backlog, Parked, Done
    const columnOrder = ['ready', 'in-progress', 'review', 'backlog', 'parked', 'done'];
  ...
  const handleVoiceCommand = useCallback((command) => {
    ...
    if (command === 'wake') {
      const msg = getRandomInsight('listening');
      setArgusMessage(msg);
      speak(msg);
      return;
    }
    ...
    // What's blocked
    if (lower.includes('blocked') && !lower.includes('unblock')) {
  ...
  const { isListening, isAwake, transcript, startListening, stopListening } = useArgusListener(handleVoiceCommand);
      ...
      <header style={styles.header}>
        <div style={styles.headerLeft}>
          <div style={styles.logoContainer}>
            <div style={{...styles.argusLogo, ...(isSpeaking ? styles.argusLogoSpeaking : {}), ...(isAwake ? styles.argusLogoAwake : {})}}>
      ...
      {showArgusPanel && (
        <div style={{...styles.argusPanel, ...(isListening ? styles.argusPanelListening : {})}}>
          <div style={styles.argusPanelContent}>
            <span style={{...styles.argusPanelEye, ...(isSpeaking ? styles.argusPanelEyeSpeaking : {}), ...(isAwake ? styles.argusPanelEyeAwake : {})}}>👁</span>
            <div style={styles.argusPanelText}>
              <p style={styles.argusPanelMessage}>{argusMessage}</p>
              {isListening && transcript && <p style={styles.transcriptText}>🎤 {transcript}</p>}
              {isAwake && !transcript && <p style={styles.awakeText}>🎤 Listening...</p>}
            </div>
          </div>
      ...
      {showVoiceSettings && (
        <div style={styles.voiceSettingsPanel}>
          ...
          <div style={styles.commandHints}>
            <strong>Voice:</strong> "Argus, move task 5 to done" • "Argus, status" • "Argus, what is task 3" • "Argus, block task 7" • "Argus, focus"
          </div>
        </div>
      )}
...
const styles = {
  ...
  argusLogoAwake: { boxShadow: '0 0 30px rgba(34, 197, 94, 0.8), 0 0 60px rgba(34, 197, 94, 0.4)', background: 'linear-gradient(135deg, #166534 0%, #22c55e 100%)' },
  ...
  argusPanelListening: { background: 'linear-gradient(135deg, rgba(22, 101, 52, 0.2) 0%, rgba(34, 197, 94, 0.2) 100%)', borderBottom: '1px solid rgba(34, 197, 94, 0.3)' },
  ...
  argusPanelEyeAwake: { filter: 'drop-shadow(0 0 8px rgba(34, 197, 94, 0.8))' },
  argusPanelMessage: { margin: 0, fontSize: '13px', color: '#c4b5fd', fontStyle: 'italic' },
  transcriptText: { margin: '4px 0 0 0', fontSize: '11px', color: '#86efac', fontFamily: 'monospace' },
